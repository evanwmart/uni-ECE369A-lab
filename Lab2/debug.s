...
# $t0 stores the character from str currently being compared to char.
# $t1 is used as the index to iterate through str.
# $t2 is used to count the number of characters in str that match char.
# $t3 is used to store char.
# 2. Currently, the program is stuck in an infinite loop. Make use of 
#    breakpoints to locate, and correct the error.
# line "add  $t1, $t2, 1" in con was changes to "add  $t1, $t1, 1" so that the index was properly incremented as opposed to remaining at 1.
